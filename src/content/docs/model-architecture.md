...
**[xHC](https://arxiv.org/abs/2607.14530) (Expanded Hyper-Connections)** — The next step in the hyper-connections family: scales the residual stream to 16 parallel streams (MHC stops at ~4) by feeding the streams richer inputs (causal depthwise convolution over neighboring tokens) and updating only 4 of the 16 per layer while reading from all — keeping the cost of many streams manageable. An **xHC-Flash** variant shares operations across layers for memory-efficient deployment.
