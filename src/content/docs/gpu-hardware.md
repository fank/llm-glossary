...
**[SM / compute capability](https://developer.nvidia.com/cuda-gpus) (sm_90, SM120, SM121a…)** — NVIDIA's version number for a GPU's instruction set. Software compiled for one may not run on another.
...
**[DGX Spark](https://www.nvidia.com/en-us/products/workstations/dgx-spark/) / GB10 / HP ZGX Nano G1n** — NVIDIA's small Grace-Blackwell desktop AI box (HP sells a variant as ZGX Nano). **Grace** = the ARM CPU part; the GPU and CPU share **unified memory** (128 GB LPDDR5X, ~273 GB/s — much slower than HBM, so decode is slower than on datacenter GPUs).
...
**[MI300X](https://www.amd.com/en/products/accelerators/instinct/mi300/mi300x.html) / Instinct** — AMD's datacenter accelerator line (Instinct); the MI300X's 192 GB HBM3 per GPU (vs. 80 GB on an H100) makes big models fit on fewer cards. Runs on the ROCm stack below, not CUDA.
...
- **[CUTLASS](https://github.com/NVIDIA/cutlass) / [FlashInfer](https://github.com/flashinfer-ai/flashinfer) / [FlashAttention](https://arxiv.org/abs/2205.14135) (flash_attn) / Marlin / [Triton](https://github.com/triton-lang/triton) / TileLang / CuteDSL / TRT-LLM kernels** — kernel libraries/backends. vLLM picks among them (`--attention-backend`, `moe_backend`); mismatches cause real incompatibilities (e.g. DFlash needs FLASH_ATTN but fp8 KV needs FLASHINFER). **[Marlin](https://arxiv.org/abs/2408.11743)** also does fast on-the-fly dequantization of 4-bit weights on GPUs without native FP4.
- **PTX / NVVM / NVCC / CUDA** — NVIDIA's GPU assembly / compiler / toolchain underneath everything.

**cudagraph (CUDA graph) / `--enforce-eager`** — Recording a whole GPU step once and replaying it, removing per-step launch overhead. `--enforce-eager` disables this (slower, but works around capture bugs). **[torch.compile](https://pytorch.org/docs/main/torch.compiler.html) / `-O3`** — PyTorch's compiler producing fused, optimized kernels at startup.
