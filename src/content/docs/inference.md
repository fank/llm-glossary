...
**CPU offloading / hybrid CPU-GPU inference** — When a model doesn't fit in GPU memory, part of it lives in system RAM and runs on (or streams through) the CPU. The classic knob is layer-granular: llama.cpp's `-ngl` sets how many layers go to the GPU, the rest stay on CPU — simple, but treats all layers as equal. Newer research systems like [ATSInfer](https://arxiv.org/abs/2607.10183) schedule at *tensor* granularity instead (individual tensors differ a lot in size and how often they're read) and adapt placement and transfers to current load, claiming up to ~3× faster decode than layer-level offloading on consumer hardware. Always slower than fitting the whole model on GPU — PCIe bandwidth becomes the ceiling.
